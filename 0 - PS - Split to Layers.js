...
function main() {
    ...
    if(allLayers) {
        for(var i = 0; i < activeDocument.artLayers.length; i++) {
            targets.push(activeDocument.artLayers[i]);
        }
    }else{
        targets.push(activeDocument.activeLayer);
    }

    for(var i in targets) {
        activeDocument.activeLayer = targets[i];

        separateActiveLayer();

        // The script leaves behind the original layer. Delete it.
        targets[i].remove();
    }
}

function separateActiveLayer() {
...
function createDialog(){
    ...
    dlg.all = dlg.add('group')
    dlg.all.orientation = 'row';
    dlg.all.chkbox = dlg.all.add('checkbox', undefined, 'All Layers')
            dlg.all.chkbox.value = allLayers;
    ...
    allLayers=dlg.all.chkbox.value
