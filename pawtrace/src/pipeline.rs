...
/// Every config value [`simplify_paths`] reads: the pass tolerance and the
/// corner threshold's input.
#[derive(Debug, Clone, PartialEq)]
...
pub fn run(
    ...
    let (alpha, regs) = if let Some(color) = raster::uniform_color(&src, cfg.alpha_threshold) {
        ...
        let prep = timing::PREP.time(|| raster::prepare(&src, &raster::PrepParams::of(cfg)));
...
fn surviving_shapes(plan: &regions::MergePlan, alpha: &GrayImage, cfg: &ShapeParams) -> Vec<Shape> {
    use rayon::prelude::*;
                ...
                let mask = if children[si].is_empty() {
                    ...
                    let mut mask = GrayImage::new(bw, y1 - y0 + 3);
                    {
                        ...
                        while let Some(mi) = stack.pop() {
                            ...
                            for &(px, py) in &r.pixels {
                                mraw[((r.y0 + py - y0 + 1) * bw + (r.x0 + px - x0 + 1)) as usize] =
                                    255;
                            }
                        }
                    }
...
fn slack_mask(
    ...
    for my in 0..bh {
        for mx in 0..bw {
            if mask.as_raw()[(my * bw + mx) as usize] == 0 {
                continue;
            }
            ...
            let touches = [(-1i64, 0i64), (1, 0), (0, -1), (0, 1)]
                ...
                .any(|(dx, dy)| {
                    ...
                    if nmx >= 0
                        ...
                        && nmx < bw as i64
                        && nmy < bh as i64
                        && mask.as_raw()[(nmy as u32 * bw + nmx as u32) as usize] != 0
                    {
                        return false;
                    }
            ...
            if touches {
                out.as_mut()[(my * bw + mx) as usize] = 255;
            }
        }
    }
...
pub fn crop_to_alpha(src: &RgbaImage, cfg: &Config) -> Option<(RgbaImage, u32, u32)> {
    ...
    for y in 0..h {
        let row =
            &src.as_raw()[(y as usize) * (w as usize) * 4..(y as usize + 1) * (w as usize) * 4];
...
mod tests {
    ...
    fn surviving_shapes_keeps_a_ring_hole_open_over_an_enclosed_survivor() {
        ...
        let regs = regions::segment_absorbed(&quant, &alpha, &regions::SegmentParams::of(&cfg));
