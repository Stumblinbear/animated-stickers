...
pub fn applies(view: StripView, _sub: Option<SubView>) -> bool {
    matches!(view, StripView::Document | StripView::Phase(Phase::Shapes | Phase::Curves))
}
...
pub fn drag(app: &mut App, p: Point) -> Task<Msg> {
    let Some(view) = app.session().filter(|s| !s.selection.is_empty()).map(|s| s.view) else {
        return Task::none();
    };
...
fn pin_at(app: &mut App, p: Point, view: StripView, toggle: bool) -> Task<Msg> {
    ...
    // diffs against them. `keys.regions` folds only up through segmentation, so
    // it is pins-independent, but `of` still needs them for the full key.
    let old: Vec<[u32; 2]> =
        app.doc().and_then(|d| d.inputs.get(&sel)).map(|i| i.pins.clone()).unwrap_or_default();
    let keys = StageKeys::of(&sess.cfg, &old);
    let Some(regs) = sess.memo.peek_regions(sel, keys.regions) else {
        return Task::none();
    };
