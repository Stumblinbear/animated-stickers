...
/// config field `trace::trace_mask` reads.
fn shape_key(cfg: &Config, mask: &GrayImage, slack: Option<&GrayImage>) -> u64 {
...
pub(super) fn trace_shapes_memo(
    ...
    let fresh: Vec<(usize, Arc<Vec<TracedPath>>)> = shapes
        ...
        .map(|(i, (_, mask, slack, _))| {
            (i, Arc::new(trace::trace_mask(mask, cfg, slack.as_ref())))
        })
        .collect();
    {
        let mut c = cache.lock().unwrap();
...
mod tests {
    ...
    fn memoed_trace_is_identical_cold_and_warm() {
        ...
        let shapes = pipeline::planned_shapes(&plan, &alpha, &cfg);
        let plain = pipeline::trace_planned(&plan, &alpha, &cfg);

        let c = cache();
        let cold = trace_shapes_memo(&c, &shapes, &cfg);
        let warm = trace_shapes_memo(&c, &shapes, &cfg);
    ...
    fn pin_toggle_refits_only_the_pinned_shape() {
        ...
        let plan = regions::merge_plan(&regs, &alpha, &cfg, 512, &[]);
        let shapes = pipeline::planned_shapes(&plan, &alpha, &cfg);
        trace_shapes_memo(&c, &shapes, &cfg);
        ...
        let memoed = trace_shapes_memo(&c, &shapes, &cfg);
