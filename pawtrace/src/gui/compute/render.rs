...
/// Fates view: each region in its own quantized color, so the image reads as
...
/// dark ring.
pub(super) fn region_fates_handle(
    regs: &[regions::Region],
    (w, h): (u32, u32),
    fates: &[regions::Fate],
    pins: &[(u32, u32)],
    ...
    let blend = |base: [u8; 3], tint: [u8; 3]| -> [u8; 3] {
        std::array::from_fn(|k| (base[k] as f32 * (1.0 - TINT) + tint[k] as f32 * TINT) as u8)
    };
    ...
    // Marker size follows the image so it stays visible zoomed out.
