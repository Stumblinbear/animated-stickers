...
/// Every config value [`segment_absorbed`] (and its absorption and
/// stroke-merge passes) reads. `scale` sizes the band-thickness ceilings,
/// which are in scaled px.
#[derive(Debug, Clone, PartialEq)]
pub struct SegmentParams {
    pub absorb_dist: f32,
    pub absorb_aggr: f32,
    pub stroke_merge_dist: f32,
    pub stroke_merge_width: f32,
    pub scale: u32,
}
...
impl SegmentParams {
    pub fn of(cfg: &Config) -> Self {
        Self {
            absorb_dist: cfg.absorb_dist,
            absorb_aggr: cfg.absorb_aggr,
            stroke_merge_dist: cfg.stroke_merge_dist,
            stroke_merge_width: cfg.stroke_merge_width,
            scale: cfg.scale,
        }
    }
}

/// Every config value [`merge_plan`] reads: the two inputs of the speckle
/// floor ([`PlanParams::turdsize`]).
#[derive(Debug, Clone, PartialEq)]
...
impl PlanParams {
    ...
    /// [`crate::config::turdsize`] at this `detail` and `scale`; `dim` =
    /// max(document W, H).
    pub fn turdsize(&self, dim: u32) -> u32 {
        crate::config::turdsize(self.detail, self.scale, dim)
    }
}
...
pub fn merge_speckles(
    ...
    gather_speckle_merged(regs, &roots, &colors)
        .into_values()
        .collect()
}
...
pub fn merge_plan(
    ...
    let shapes: Vec<(GrayImage, u64)> = crate::timing::SHAPES.time(|| {
        mregs
            .par_iter()
            .map(|r| region_shape(r, alpha, floor))
            .collect()
    });
...
pub fn report_of(plan: &MergePlan) -> RegionReport {
    ...
    let areas = plan
        .roots
        .iter()
        .map(|&root| plan.areas[idx_of(root)])
        .collect();
...
fn next_worklist(nodes: &mut Vec<Node>, merges: &[(u32, u32)]) -> Vec<u32> {
    ...
    for &r in &targets {
        ...
        let nb: Vec<u32> = nodes[r as usize]
            .neighbors
            .iter()
            .map(|&(nid, _)| nid)
            .collect();
...
pub fn segment_absorbed(quant: &RgbImage, alpha: &GrayImage, cfg: &SegmentParams) -> Vec<Region> {
...
fn absorb(regions: Vec<Region>, w: u32, h: u32, cfg: &SegmentParams) -> Vec<Region> {
    ...
    for _ in 0..8 {
        ...
        for &id in &worklist {
            ...
            let target = if da < cfg.absorb_dist {
                neigh[0].0
            } else {
                neigh[1].0
            };
...
fn merge_strokes(regions: Vec<Region>, w: u32, h: u32, cfg: &SegmentParams) -> Vec<Region> {
    ...
    for y in 0..bh {
        for x in 0..bw {
            ...
            while let Some((hx, hy)) = q.pop() {
                ...
                let mut visit = |nx: u32, ny: u32, q: &mut Vec<(u32, u32)>| {
                    if hole_id[idx(nx, ny)] == 0 && !outside[idx(nx, ny)] && m[idx(nx, ny)] == 0 {
...
mod tests {
    ...
    fn merge_strokes_reunites_a_quantization_split_outline() {
        ...
        let merged = merge_strokes(regs, 12, 4, &SegmentParams::of(&cfg));
    ...
    fn merge_strokes_leaves_wide_bands_apart() {
        ...
        assert_eq!(merge_strokes(regs, 8, 8, &SegmentParams::of(&cfg)).len(), 2);
    }
