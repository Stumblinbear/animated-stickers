...
fn run_layer(
    ...
    let t = Instant::now();
    let plan = regions::merge_plan(&regs, &alpha, cfg, doc_dim, &pins);
    times.add(6, t);
    ...
    times.add(6, t);
    let t = Instant::now();
    let mut out = pipeline::simplify_paths(traced, cfg);
    times.add(7, t);
        ...
        for iter in 0..=ITERS {
            ...
            let traced: Vec<Traced> = layers
                ...
                .map(|(name, img)| {
                    ...
                    Traced { name: name.clone(), stroke: output::stroke_of(&cfg), colors }
                })
                .collect();
